...
def delete_a_car():
    ...
    try:
        ...
        list_index = primer1_modul.find_for_inevnory_id(deletion_id)
        if list_index > -1:
            cars = primer1_modul.load_cars()
            del cars[list_index]
            primer1_modul.save_cars(cars)
            print("The cars has been successfully deleted!")
        else:
            print(f"There is no car with invenotry ID: {deletion_id}")
    except ValueError:
